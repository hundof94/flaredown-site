...
export function Footer() {
  return (
    <footer className="bg-panel text-ink-2 mt-20 border-t border-ink/10">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10">
          <div className="col-span-1">
            <Link href="/" className="flex items-center gap-2 font-display font-bold text-lg text-ink mb-3">
              ...
              <ul className="space-y-2">
                {links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="text-sm text-ink-3 hover:text-ink transition-colors duration-200"
                    >
                      {link.label}
                    </Link>
                  </li>
        ...
        <div className="mt-12 pt-8 border-t border-ink/10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <p className="text-xs text-ink-3">
            © {new Date().getFullYear()} FlareDown. All rights reserved.
          </p>
          <p className="text-xs text-ink-3 max-w-md text-right">
